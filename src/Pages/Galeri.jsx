      ...
      <div className="flex-grow container mx-auto px-4 py-8">
        ...
        {gallery.length > 0 ? (
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {gallery.map((image) => (
              <div key={image.id} className="cursor-pointer" onClick={() => openModal(`http://localhost:3002/uploads/${image.image}`, image.caption)}>
                <img src={`http://localhost:3002/uploads/${image.image}`} alt={image.title} className="w-full h-56 object-cover rounded-lg shadow" />
